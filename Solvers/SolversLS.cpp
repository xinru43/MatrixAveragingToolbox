	...
	void SolversLS::Run(void)
	{
		...
		if (Debug >= ITERRESULT)
		{
            ...
            if (Debug >= DETAILED)
            {
                ...
                initstepsizeSeries[iter] = 0;
                stepsizeSeries[iter] = 0;
            }
		}
		bool isstop = IsStopped();

		
		if (isnan(f1) || isnan(ngf0))
        {
            isstop = true;
            printf("Stops early since the function is nan\n");
        }
		...
		while ((((! isstop) && iter < Max_Iteration) || iter < Min_Iteration) && LSstatus == SUCCESS)
		{
            ...
            if (Debug >= DETAILED)
            {
                initstepsizeSeries[iter + 1] = initiallength;
            }
