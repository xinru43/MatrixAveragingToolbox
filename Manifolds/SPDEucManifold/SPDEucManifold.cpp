...
namespace ROPTLIB{
	...
	SPDEucManifold::SPDEucManifold(integer inn)
	{
		n = inn;
		IsIntrApproach = true;
		HasHHR = false;
		UpdBetaAlone = false;
		HasLockCon = false;
		name.assign("SPDEucManifold");
		IntrinsicDim = n * (n + 1) / 2;
		ExtrinsicDim = n * n;
		EMPTYEXTR = new SPDEucVector(n, n);
		EMPTYINTR = new SPDEucVector(IntrinsicDim, 1);
	};

	SPDEucManifold::~SPDEucManifold(void)
	{
		delete EMPTYEXTR;
		delete EMPTYINTR;
	};

	void SPDEucManifold::CheckParams(void) const
	{
	...
	void SPDEucManifold::EucGradToGrad(Variable *x, Vector *egf, Vector *gf, const Problem *prob) const
	{
        egf->CopyTo(gf);
...
// 		delete[] tmp;
	...
	void SPDEucManifold::EucHvToHv(Variable *x, Vector *etax, Vector *exix, Vector* xix, const Problem *prob) const
	{
		printf("warning:SPDEucManifold::EucHvToHv has not been done!\n");
		exix->CopyTo(xix);
	};

	void SPDEucManifold::CholeskyRepresentation(Variable *x) const
	{
		...
		if (info != 0)
		{
			printf("Warning: SPDEucManifold::CholeskyRepresentation fails with info:%d!\n", info);
		}
	};
    

	void SPDEucManifold::ExtrProjection(Variable *x, Vector *etax, Vector *result) const
	{
	...
	void SPDEucManifold::ObtainIntr(Variable *x, Vector *etax, Vector *result) const
...
// 		if (info != 0)
// 		{
// 			printf("warning: SPDEucManifold::ObtainIntr fails with info:%d!\n", info);
// 		}
...
// 		}
	...
	void SPDEucManifold::ObtainExtr(Variable *x, Vector *intretax, Vector *result) const
	{
// 		if (!x->TempDataExist("L"))
// 		{
// 			CholeskyRepresentation(x);
...
// 		delete[] E;
    ...
    double SPDEucManifold::EucDist(Variable *x1, Variable *x2) const
    {
        integer N = n, length = n * n;
        double result = 0.0, error;
        const double *x1M = x1->ObtainReadData();
        const double *x2M = x2->ObtainReadData();
...
//             error = const_cast<double *>(x1M + i) - const_cast<double *>(x2M + i);
	...
	double SPDEucManifold::Dist(Variable *x1, Variable *x2) const
	{
	...
	void SPDEucManifold::Retraction(Variable *x, Vector *etax, Variable *result, double stepsize) const
	{
		if (!x->TempDataExist("L"))
		{
			CholeskyRepresentation(x);
		...
		if (info != 0)
		{
			printf("warning: SPDEucManifold::Retraction fails with info:%d!\n", info);
		}
		double *resultTV = result->ObtainWriteEntireData();
	...
	void SPDEucManifold::DiffRetraction(Variable *x, Vector *etax, Variable *y, Vector *xix, Vector *result, bool IsEtaXiSameDir) const
	...
	void SPDEucManifold::coTangentVector(Variable *x, Vector *etax, Variable *y, Vector *xiy, Vector *result) const
	{
		printf("SPDEucManifold::coTangentVector has not been done!\n");
		xiy->CopyTo(result);
	};

	double SPDEucManifold::Beta(Variable *x, Vector *etax) const
	{
