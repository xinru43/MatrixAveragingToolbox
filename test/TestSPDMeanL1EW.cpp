...
{
    printf("============= Median computation by EW method =============\n");
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    printf("============= Median computation by EW method =============\n");
    // deal with input
    double *Ls, *initial_X, *Xtrue;
    integer n, num, maxiter, Debug;
    double TOL, stepsize, alpha;
    Ls = mxGetPr(prhs[0]);
    ...
    stepsize = static_cast<double>(mxGetScalar(prhs[6]));
...
//     time start
    ...
    integer length = n * n, N = n, info, enditer;
    double minval, negone = -1.0, coefa;
...
//   --------------  check if dist(Xk, Ai) == 0: find the smallest dist(Xk, Ai)  --------------
        minval = coef[0];
        idx = 0;
        for (integer i = 1; i < num; i++)
        {
...
//          --------------  compute eta  --------------
        ...
        if (minval > 1e-16)
        {
            ...
            for (integer i = 0; i < num; i++)
            {
                ...
                dgemm_(GLOBAL::N, GLOBAL::T, &N, &N, &N, &GLOBAL::DONE, logLXL + n * n * i, &N,
                ...
                coefa = 1.0/coef[i];
                dgemm_(GLOBAL::N, GLOBAL::N, &N, &N, &N, &coefa, X1, &N, tmp, &N, &GLOBAL::DONE, eta, &N);
            }
