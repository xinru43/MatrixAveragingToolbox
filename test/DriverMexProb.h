...
void ParseSolverParamsAndOptimizing(const mxArray *SolverParams, Problem *Prob, Variable *initialX, Variable *soln, mxArray **&plhs)
{
    ...
    plhs[17] = mxCreateDoubleMatrix(lengthSeries, 1, mxREAL);
    plhs[18] = mxCreateDoubleMatrix(lengthSeries, 1, mxREAL);
...
// 		delete TV0;
	...
	double *plhsfun = mxGetPr(plhs[12]), *plhsgrad = mxGetPr(plhs[13]), *plhstime = mxGetPr(plhs[14]), 
            *plhsdist = mxGetPr(plhs[15]), *plhsinitstepsize = mxGetPr(plhs[17]), *plhsstepsize = mxGetPr(plhs[18]);
	for (integer i = 0; i < lengthSeries; i++)
	{
        ...
        plhsinitstepsize[i] = solver->GetinitstepsizeSeries()[i];
        plhsstepsize[i] = solver->GetstepsizeSeries()[i];
	}
