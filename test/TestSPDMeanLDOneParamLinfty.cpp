...
#include "TestSPDMeanLDOneParamLinfty.h"
...
#if !defined(MATLAB_MEX_FILE) && defined(TESTSPDMEANLDONEPARAMLINFTY)
...
void testSPDMeanLDOneParamLinfty(void)
{
	...
	// Define the problem
	SPDMeanLDOneParamLinfty Prob(Ls, LDAs, n, num, alpha);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	if (nrhs >= 7)
	{
		double *soln = mxGetPr(prhs[6]); /*soln: n by p*/
	...
	SPDMeanLDOneParamLinfty Prob(Ls, LDAs, n, N, alpha);
	Prob.SetDomain(&Domain);
    ...
    mexProblem::ObtainElementFromMxArray(SPDX, plhs[0]);
	...
	integer length = 0;
	double testzero = 0;
    	plhs[20] = mxCreateDoubleScalar(testzero);
	for (integer i = 0; i < numdim; i++)
		delete dgfs[i];
	delete[] dgfs;
