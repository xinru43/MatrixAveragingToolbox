...
#include "TestSPDMeanSymmLDOneParam.h"
...
#if !defined(MATLAB_MEX_FILE) && defined(TESTSPDMEANSYMMLDONEPARAM)
...
void testSPDMeanSymmLDOneParam(void)
{
	...
	// Define the problem
	SPDMeanSymmLDOneParam Prob(Ls, n, num, alpha);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	if (mxGetM(prhs[2]) != n || mxGetN(prhs[2]) != n)
	{
	...
	SPDMeanSymmLDOneParam Prob(Ls, n, N, alpha);
	Prob.SetDomain(&Domain);
	Domain.SetHasHHR((HasHHR != 0));
	ParseSolverParamsAndOptimizing(prhs[4], &Prob, SPDX, SPDsoln, plhs);
