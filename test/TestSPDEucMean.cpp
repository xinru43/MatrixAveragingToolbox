...
void testSPDEucMean(void)
{
	...
	/*Randomly generate a point on the SPDEuc manifold*/
	integer n = 100, num = 4;
	SPDEucVariable SPDX(n);
	...
	for (integer i = 0; i < num; i++)
	{
		...
		if (info != 0)
		{
			printf("Warning: TestSPDEucMean Cholesky decomposition fails with info:%d!\n", info);
		}
		for (integer j = 0; j < n; j++)
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	SPDEucVariable *SPDX = new SPDEucVariable(n);
	double *initialX = SPDX->ObtainWriteEntireData();
	...
	SPDEucVariable *SPDsoln = nullptr;
	if (nrhs >= 5)
	{
		double *soln = mxGetPr(prhs[4]); /*soln: n by p*/
		SPDsoln = new SPDEucVariable(n);
