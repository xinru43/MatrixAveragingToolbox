...
#include "SPDMeanSymmLDOneParam.h"

/*Define the namespace*/
namespace ROPTLIB{
    
    SPDMeanSymmLDOneParam::SPDMeanSymmLDOneParam(double *inLs, integer inn, integer innum, double inalpha)
    {
        Ls = inLs;
        n = inn;
        num = innum;
    ...
    SPDMeanSymmLDOneParam::~SPDMeanSymmLDOneParam(void)
    {
    };
    
    
    double SPDMeanSymmLDOneParam::f(Variable *x) const
    {
        ...
        result = result - detX;
        result = (result * 2.0)/(1.0 - alpha * alpha);
    ...
    void SPDMeanSymmLDOneParam::RieGrad(Variable *x, Vector *gf) const
    {    
        double a1 = (1.0 - alpha)/2.0, a2 = (1.0 + alpha)/2.0;
...
//             for(integer j = 0; j < n * n; j++) gfVT[j] = gfVT[j] + tmp[j];
        ...
        double scalar = 2.0/(1.0 - alpha * alpha);
...
//----------------------------------- Riemannian Hessian -----------------------------------
    void SPDMeanSymmLDOneParam::RieHessianEta(Variable *x, Vector *etax, Vector *xix) const
    {
...
//     void SPDMeanLDOneParam::RieHessianEig(Variable *x, double *EigVal) const
//     {
