...
#include "SphereHessianStein.h"

/*Define the namespace*/
namespace ROPTLIB{

	SphereHessianStein::SphereHessianStein(double *inCs, double *inDs, integer inn, integer innum)
	{
        Cs = inCs;
        Ds = inDs;
        n = inn;
    ...
    SphereHessianStein::~SphereHessianStein(void)
	{
	};

double SphereHessianStein::f(Variable *x) const
	{
...
//         printf("f: %.4f\n", result);
	...
	void SphereHessianStein::EucGrad(Variable *x, Vector *egf) const
	{
        const double *xxM = x->ObtainReadData();
...
//            dgemm_(GLOBAL::N, GLOBAL::N, &N, &N, &N, &GLOBAL::DONE, const_cast<double *> (CxD + n * n * i), &N, I, &N, &GLOBAL::DONE, egfTV, &N);
        ...
        dgemm_(GLOBAL::T, GLOBAL::N, &N, &N, &N, &GLOBAL::DONE, tmp, &N, I, &N, &GLOBAL::DZERO, egfTV, &N);
	...
	void SphereHessianStein::EucHessianEta(Variable *x, Vector *etax, Vector *exix) const
	{
    ...
    double SphereHessianStein::dis(Variable *x) const
    {
        return 0;
    };
    
    double SphereHessianStein::distwo(Variable *x, Variable *y) const
    {
        return 0;
    };
