...
#include "SphereHessianSteinMax.h"

/*Define the namespace*/
namespace ROPTLIB{

	SphereHessianSteinMax::SphereHessianSteinMax(double *inCs, double *inDs, integer inn, integer innum)
	{
        Cs = inCs;
        Ds = inDs;
        n = inn;
    ...
    SphereHessianSteinMax::~SphereHessianSteinMax(void)
	{
	};

double SphereHessianSteinMax::f(Variable *x) const
	{
...
//         printf("f: %.4f\n", result);
	...
	void SphereHessianSteinMax::EucGrad(Variable *x, Vector *egf) const
	{
        const double *xxM = x->ObtainReadData();
...
//            dgemm_(GLOBAL::N, GLOBAL::N, &N, &N, &N, &GLOBAL::DONE, const_cast<double *> (CxD + n * n * i), &N, I, &N, &GLOBAL::DONE, egfTV, &N);
        ...
        double negone = -1.0;
        dgemm_(GLOBAL::T, GLOBAL::N, &N, &N, &N, &negone, tmp, &N, I, &N, &GLOBAL::DZERO, egfTV, &N);
	...
	void SphereHessianSteinMax::EucHessianEta(Variable *x, Vector *etax, Vector *exix) const
	{
