...
namespace ROPTLIB{
    ...
    double SPDMeanLDOneParamLinfty::f(Variable *x) const
    {
        ...
        for (integer i = 0; i < num; i++)
        {
            coef1[i] = coef1[i] - a1 * LDAs[i] - a2 * detX;
        }
    ...
    void SPDMeanLDOneParamLinfty::RieGrad(Variable *x, Vector *gf) const
    {
        ...
        // compute inv(LplusX) = inv(LplusX)^T * inv(LplusX) -> gfVT
        dgemm_(GLOBAL::T, GLOBAL::N, &N, &N, &N, &GLOBAL::DONE, iLplusX, &N, iLplusX, &N, &GLOBAL::DZERO, gfVT, &N);
